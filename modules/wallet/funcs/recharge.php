...
$request_recharge = [];
// Nạp custom từ url dạng amount=100000-VND
$request_recharge['pay_amount'] = nv_substr($nv_Request->get_title('amount', 'get', ''), 0, 100);
// Chọn số tài khoản nhận ATM, SePay
$request_recharge['atm_account_no'] = nv_substr($nv_Request->get_title('no', 'get', ''), 0, 50);
// Message nạp ATM, SePay
$request_recharge['atm_info'] = nv_substr($nv_Request->get_title('i', 'get', ''), 0, 25);
// Message nạp chung
$request_recharge['info'] = nv_substr($nv_Request->get_title('info', 'get', ''), 0, 250);

if (!empty($request_recharge['pay_amount'])) {
    $page_url .= '&amp;amount=' . urlencode($request_recharge['pay_amount']);
}
if (!empty($request_recharge['atm_account_no'])) {
    $page_url .= '&amp;no=' . urlencode($request_recharge['atm_account_no']);
}
if (!empty($request_recharge['atm_info'])) {
    $page_url .= '&amp;i=' . urlencode($request_recharge['atm_info']);
}
if (!empty($request_recharge['info'])) {
    $page_url .= '&amp;info=' . urlencode($request_recharge['info']);
}
...
if ($payment == 'sms') {
    if (!defined('NV_IS_USER')) {
        $linkdirect = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=users&amp;' . NV_OP_VARIABLE . '=login&nv_redirect=' . nv_redirect_encrypt(str_replace('&amp;', '&', nv_url_rewrite($page_url, true)));
...
if ($payment == 'epay') {
    ...
    require NV_ROOTDIR . '/modules/' . $module_file . '/payment/epay.complete.php';
} elseif ($payment == 'gamebank') {
    // Nạp qua Gamebank xử lý riêng
    require NV_ROOTDIR . '/modules/' . $module_file . '/payment/gamebank.checkout_url.php';
} elseif ($payment == 'vnptepay') {
    // Nạp qua VNPT Epay xử lý riêng
    require NV_ROOTDIR . '/modules/' . $module_file . '/payment/vnptepay.checkout_url.php';
}
...
$array_banks = [];
$is_vietqr = false;
if (in_array($payment, ['ATM', 'VietQR', 'sepay']) and !empty($payment_config['acq_id']) and !empty($payment_config['account_no']) and !empty($payment_config['account_name'])) {
    $is_vietqr = true;
...
if ($checkss == md5($payment . $global_config['sitekey'] . session_id())) {
    ...
    $atm_error = $vietrq_error = $sepay_error = '';
    if ($payment == 'ATM') {
    ...
    if (!empty($post['customer_email']) and !empty($check_valid_email)) {
        ...
        if (empty($post['id'])) {
            ...
            $post['transaction_code'] = sprintf('GD%010s', $post['id']);
    ...
    $post['money_unit'] = '';
    $post['check_term'] = 0;
    $post['transaction_info'] = $request_recharge['info'];
     ...
     */
    $pay_money = '';
    if (preg_match('/^([0-9\.]+)\-([A-Z]{3})$/', $request_recharge['pay_amount'], $m)) {
        if (!isset($global_array_money_sys[$m[2]])) {
            $request_recharge['pay_amount'] = 0;
        } else {
            $pay_money = $m[2];
            $request_recharge['pay_amount'] = $m[1];
        }
    } else {
        $request_recharge['pay_amount'] = 0;
    }
    if (!empty($request_recharge['pay_amount']) and in_array($pay_money, $row_payment['currency_support'])) {
        $post['money_unit'] = $pay_money;
        $list_money = array_filter(explode(',', $module_config[$module_name]['minimum_amount'][$post['money_unit']]));
        if (in_array($request_recharge['pay_amount'], $list_money)) {
            $post['money_amount'] = $request_recharge['pay_amount'];
        } elseif (empty($list_money) or min($list_money) <= $request_recharge['pay_amount']) {
            $post['money_other'] = $request_recharge['pay_amount'];
        }
    }
    ...
    if ($payment == 'ATM' or $payment == 'VietQR') {
        ...
        if (!empty($request_recharge['atm_account_no'])) {
            foreach ($payment_config['account_no'] as $key => $account_no) {
                if ($account_no === $request_recharge['atm_account_no']) {
                    $post['atm_acq'] = $key;
                    break;
                }
            }
    ...
    if (in_array($payment, ['ATM', 'VietQR', 'sepay'])) {
        ...
        $post['transaction_info'] = $request_recharge['atm_info'];
    }
    if ($payment == 'sepay') {
        $post['to_account'] = '';
